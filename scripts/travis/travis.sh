...
function push_results() {
    ...
    REMOTE_LOGFILE="/tmp/${TESTSUITE}_${TIMESTAMP}.log"
    
    echo "Uploading results ..."
    execute "mv ${LOGFILE} ${REMOTE_LOGFILE}; s3cmd --access_key=${S3_KEY} --secret_key=${S3_SECRET} --host=${S3_HOST} --no-ssl put ${REMOTE_LOGFILE} s3://${S3_LOGS_BUCKET}"
}
