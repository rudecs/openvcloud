defineApiStub = function($httpBackend){
    ...
    var images = [
             ...
             {'id':1, 'name':'Windows', 'description': 'A windows 2012 server', "type": "Windows"}
             ];

    var sizes = [
              {'id':0, 'name':'Small', 'CU':1, 'disksize': 20},
              {'id':1, 'name':'Medium', 'CU':2, 'disksize': 40},
              {'id':2, 'name':'Big', 'CU':4, 'disksize': 100}];

    var actionlist =  {'stop':'HALTED', 'start': 'RUNNING'};
    ...
    $httpBackend.whenGET(/^\/machines\/create\?authkey\=yep123456789/).respond(function(method, url, data) {
            ...
            var machine = {"status": "RUNNING", "cloudspaceId":params.cloudspaceId,"hostname": params.name, "description": params.description, "name": params.name, "nics":[], "sizeId":params.sizeId, "imageId": params.imageId, "id": id};
            machines[id] = machine;
            return [200, id];
