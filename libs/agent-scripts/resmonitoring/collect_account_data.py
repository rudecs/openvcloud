...
def action():
    ...
    """
    try:
        base_path_active = "/opt/var/resourcetracking/active"
        base_path_collected = "/opt/var/resourcetracking/collected"
                ...
                for f in tar.getmembers():
                    if f.name.endswith(".bin"):
                        accountid, year, month, day, hour, name = re.findall(
                            "opt/var/resourcetracking/active/([\d]+)/([\d]+)/([\d]+)/([\d]+)/([\d]+)/([\d]+.bin)", f.name)[0]
                        try:
