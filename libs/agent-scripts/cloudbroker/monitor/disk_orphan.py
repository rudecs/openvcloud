            ...
            if file_.endswith('.raw'):
                if 'routeros' in file_:
                    ...
                    if networkid not in activenetworks:
                        results.append({'state': 'ERROR', 'category': 'Orphanage', 'message': 'Found orphan disk %s' % fullpath})
                else:
                    diskstatus = diskmap.get(fullpath, 'DESTROYED')
                    if diskstatus == 'DESTROYED':
                        results.append({'state': 'ERROR', 'category': 'Orphanage', 'message': 'Found orphan disk %s' % fullpath})
            elif file_.endswith('.iso') and len(files) == 1:
                results.append({'state': 'ERROR', 'category': 'Orphanage', 'message': 'Found orphan cloud-init %s' % fullpath})
        if not files:
            results.append({'state': 'ERROR', 'category': 'Orphanage', 'message': 'Found empty folder %s' % folder})
