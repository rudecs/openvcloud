...
class CSLibvirtNodeDriver():
    ...
    def _create_disk(self, vm_id, size, image, disk_role='base'):
        ...
        return self._execute_agent_job('createdisk', vmname=vm_id, size=disksize, role='storagenode', templateguid=templateguid, pmachineip=pmachineip)
    ...
    def ex_clone(self, node, size, vmid, networkid, diskmapping):
        name = 'vm-%s' % vmid
        pmachineip = self._get_connection_ip()
        diskpaths = self._execute_agent_job('clonevolumes', name=name, machineid=node.id, role='storagenode', diskmapping=diskmapping, pmachineip=pmachineip)
        volumes = []
