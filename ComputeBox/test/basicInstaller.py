...
def install_prereqs():
    ...
    run('chmod 0600 ~/.ssh/*')
    run('hg clone --ssh "ssh -o StrictHostKeyChecking=no" ssh://hg@bitbucket.org/incubaid/jp_cloudscalers /opt/jumpscale/var/jpackages/metadata/')
    run('mv /opt/jumpscale/var/jpackages/metadata/jp_cloudscalers/unstable/* /opt/jumpscale/var/jpackages/metadata/')
